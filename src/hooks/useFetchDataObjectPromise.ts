...
const useFetchDataObjectPromise = <T>(Path: string, filters: Record<string, string> = {}): UseFetchDataReturn<T> => {
    ...
    if (!process.env.REACT_APP_MAKEUP_API_URL) {
        ...
        setLoading(false);
        return {data: null as T, loading: false, error: "Failed to read REACT_APP_MAKEUP_API_URL"};
    }
...
export default useFetchDataObjectPromise;
